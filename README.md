...
#### 문제점
- 발주/제안/채팅/주문/정산이 각각 분리되어 복잡함
- 같은 정보가 여러 화면에서 중복 노출

#### 메뉴 구조 변경
- [ ] 기존: 내발주, 받은제안, 채팅, 주문내역, 내정보
- [ ] 변경: **내발주, 새발주등록, 내정보** (3개로 압축)

#### 내발주 페이지 개편
- [ ] 상태 카드 제거 (모집중, 진행중, 받은제안 삭제)
- [ ] 2단 레이아웃: 왼쪽=발주 리스트 / 오른쪽=받은 제안 리스트
- [ ] 받은제안, 주문내역, 채팅 기능을 내발주에 통합
- [ ] 최근발주 → 받은제안 리스트로 통합

#### 채팅/거래 흐름 변경
- [ ] "거래 확정하기" → "입금 확인 요청" (구매자 전용)
  - 클릭해도 채팅창 유지
  - 상태: "입금 확인 대기"로 변경
- [ ] 거래 확정 팝업에서 구매자에게 불필요한 결제 안내 삭제
- [ ] "거래 확정" → "결제 진행하기" 버튼명 변경
  - PG 연동 후 별도 결제창 생성 예정 안내

#### 판매자용 채팅 버튼 추가
- [ ] "입금 확인 완료" 버튼 (판매자 전용)
  - 시스템 메시지: "판매자가 입금을 확인했습니다"
  - 채팅 상단에 납품일/납품예정일 고정 표기
  - 안내문구: "납품을 진행하겠습니다."
- [ ] "납품 완료" 버튼 (판매자 전용)
  - 클릭 시 상태 '거래완료' 처리 + 거래이력 저장
